...
/// <summary>
/// Infrastructure layer for SupervisorAgent that wraps AIAgent with token tracking, retry logic, and metrics.
/// Handles supervisor-specific AI invocations (intent routing, agent coordination) with exponential backoff for rate limiting.
/// </summary>
public class SupervisorFinancialsAIAgent(AIAgent agent, IConfiguration configuration, ILogger<SupervisorFinancialsAIAgent> logger)
{
    private readonly AIAgent _agent = agent ?? throw new ArgumentNullException(nameof(agent));
    private readonly IConfiguration _configuration = configuration ?? throw new ArgumentNullException(nameof(configuration));
    private readonly ILogger<SupervisorFinancialsAIAgent> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    private const int MaxRetries = 3;

    public string AgentId => _configuration["Values:AGENT_ID"] ?? _configuration["AGENT_ID"] ?? "SupervisorAgent";
    ...
    /// <summary>
    /// Main entry point for supervisor-related AI invocations (intent routing, multi-agent coordination).
    /// </summary>
    public async Task<AgentResponse> AnalyzeSupervisorRequestAsync(
        string userQuery,
        ...
        string? hopId = null)
    {
        ...
        _logger.LogInformation(
            "Starting SupervisorFinancialsAIAgent run. AgentId={AgentId}, WorkflowId={WorkflowId}, HopId={HopId}, Route={Route}, Scenario={Scenario}, ExperimentPhase={ExperimentPhase}, Deployment={DeploymentName}, Endpoint={Endpoint}, PromptLength={PromptLength}, EstimatedInputTokens={EstimatedInputTokens}",
            AgentId,
            workflowId,
            hopId,
            route,
        ...
        while (true)
        {
            try
            {
                ...
                _logger.LogInformation(
                    "SupervisorFinancialsAIAgent run completed. AgentId={AgentId}, SdkAgentInstanceId={SdkAgentInstanceId}, Deployment={DeploymentName}, ResponseId={ResponseId}, MessageCount={MessageCount}, TextLength={TextLength}",
                    AgentId,
                    response.AgentId,
                    deploymentName,
                    response.ResponseId,
                ...
                {
                    _logger.LogWarning(
                        "SupervisorFinancialsAIAgent returned empty text. AgentId={AgentId}, SdkAgentInstanceId={SdkAgentInstanceId}, Deployment={DeploymentName}, ResponseId={ResponseId}, MessageCount={MessageCount}",
                        AgentId,
                        response.AgentId,
                        deploymentName,
                        response.ResponseId,
            ...
            catch (RequestFailedException ex) when (ex.Status == 429 && retryCount < MaxRetries)
            {
                ...
                _logger.LogWarning(
                    ex,
                    "SupervisorFinancialsAIAgent throttled by Azure OpenAI. Deployment={DeploymentName}, RetryAttempt={RetryAttempt}, DelayMs={DelayMs}",
                    deploymentName,
                    retryCount,
            ...
            catch (Exception ex) when (ex.Message.Contains("429") && retryCount < MaxRetries)
            {
                ...
                _logger.LogWarning(
                    ex,
                    "SupervisorFinancialsAIAgent encountered 429-compatible exception. Deployment={DeploymentName}, RetryAttempt={RetryAttempt}, DelayMs={DelayMs}",
                    deploymentName,
                    retryCount,
                    delay.TotalMilliseconds);
                await Task.Delay(delay);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(
                    ex,
                    "SupervisorFinancialsAIAgent run failed. Deployment={DeploymentName}, Endpoint={Endpoint}, PromptLength={PromptLength}, ExceptionType={ExceptionType}",
                    deploymentName,
                    endpoint,
                    userQuery.Length,
                    ex.GetType().FullName);
